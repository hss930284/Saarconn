...
from arelements_def import root
   

from Pkg_struct import ARXMLStructure
...
def exportarxml():
    ...
    # expkg = Utilities.GetElementByPath("/SwComponentTypes/ApplSWC/ModeManagement")
    # sysTutorial = expkg.Elements.Item(ApplicationSwComponentType)
    # systemExtractDir = os.path.join(projectRootDir, "_SystemFiles", "SystemExtracts")
    
    # sdProject = SdApplication.ActiveProject
    # print(sdProject)
    # arxml_file = "E:\\sysdesk\\automation\\projects"
    # sd_project.Serializer.Export(arxml_file)


    # importSettings = sd_project.Serializer.GetNewImportSettings()
    # importSettings.SetFilePaths(arxml_file)
    ...
    # success = sd_project.Serializer.Export(importSettings)
    # if success == False:
    #     raise Exception("AUTOSAR export failed")
    # applRootDir = r"<InstallationDir>"

    

    exportscehma= SystemDeskEnums.ArSchemas.Autosar00051

    # abcd=SdProject.GetSupportedExportSchemas
    ...
    # arxml_file = [os.path.join(applRootDir, r"Templates\\abcd")]

    # arxml_file = os.path.join(Utilities.GetProjectRootDir(), "abcd.arxml")


    

    exportSettings = SdProject.Serializer.GetNewExportSettings()
    exportSettings.FilePath = projectRootDir
    exportSettings.ExportReferencedElements = True
    exportSettings.ExportImplementations = True
    
    # exportSettings.GetSupportedExportSchemas("Autosar00051")
    exportSettings.CheckSettings()

    success = SdProject.Serializer.Export(exportSettings)
    if success == False:
        raise Exception("AUTOSAR export failed")
    ...
    # fileNames="E:\sysdesk\Arxml_with_Ver\All_In_One_4_0_2.arxml"

    # Utilities.ImportAutosarFilesAtProject(fileNames, importDiagrams=True, optionShowImportDialog=False)
...
def Main():
    ...
    root_folders = arxml_structure.get_variable('root')

    autosar_folder = arxml_structure.get_variable('autosar_folder')
    ...
    # root_folders = Default_Pkg_Struct(root)


    # Convert the dictionary to an XML element
    root_element = ET.Element("autosar_folder")
    ...
    try:
        ...
        with open("output131.arxml", "wb") as f:
