...
def ApplicationSwComponentType():
    global swComponentTypesPackage, ApplSWC, CurrentSWC

    swComponentTypesPackage = rootPackage.ArPackages.Item("SwComponentTypes")

    ApplSWC= swComponentTypesPackage.ArPackages.Item("ApplSWC") # use this method >>ilSwcType = Utilities.GetElementByPath("/SwComponentTypes/IndicatorLogic/IndicatorLogic")
   
    CurrentSWC = ApplSWC.Elements.AddNewApplicationSwComponentType(current_df.iloc[2,2])

    IB = CurrentSWC.InternalBehaviors.AddNew(current_df.iloc[2,2])
