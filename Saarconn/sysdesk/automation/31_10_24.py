...
def InitEvent(CurrentRteEvent):
    CurrentEvent = CurrentInternalBehaviors.Events.AddNewInitEvent(current_df.iloc[CurrentRteEvent,Actual_CellNo_Colomn])
    # Utilities.SetDescription(tssEvent, "TimingEvent for TssPreprocessing Runnable [10 ms period].")
    CurrentEvent.StartOnEventRef = CurrentRunnable

def TimingEvent(CurrentRteEvent):
    CurrentEvent = CurrentInternalBehaviors.Events.AddNewTimingEvent(current_df.iloc[CurrentRteEvent,Actual_CellNo_Colomn])
    # Utilities.SetDescription(tssEvent, "TimingEvent for TssPreprocessing Runnable [10 ms period].")
    CurrentEvent.Period = 0.01
    CurrentEvent.StartOnEventRef = CurrentRunnable
...
# def createrteevent():
            ...
            # GetColumnValidIndex("H3", "Application Data Type")
            # firstvalueCell ,lastvaluecell
...
def createrteevent():
    # Get the values from the column corresponding to H3
    GetColumnValidIndex("H3", "Application Data Type")

        # Convert firstvalueCell and lastvaluecell to row indices
    first_row_index = int(firstvalueCell[1:]) - 1  # Convert from Excel row (1-based) to Python index (0-based)
    last_row_index = int(lastvaluecell[1:]) - 1  # Convert from Excel row (1-based) to Python index (0-based)

    # Iterate through the range from first_row_index to last_row_index
    for CurrentRowNo in range(first_row_index, last_row_index + 1):  # +1 to include the last row
        # Get the value in the current row and column specified by CellNo_Colomn
        cell_value = current_df.iloc[CurrentRowNo, Actual_CellNo_Colomn]
        
        # Check if the cell value matches 'AsynchronousServerCallReturnsEvent'
    ...
    # for CurrentRowNo in range(len(current_df)):
    #     # Get the value in the current row and column specified by CellNo_Colomn
    #     cell_value = current_df.iloc[CurrentRowNo, CellNo_Colomn]
