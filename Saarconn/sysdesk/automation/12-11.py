...
# def get_table_and_e_cell(table_value, header_phrase):
#     global global_table1_cell, global_table2_cell, global_table3_cell, global_table4_cell
#     global_table1_cell = None
#     global_table2_cell = None
#     global_table3_cell = None
#     global_table4_cell = None

#     # Define the header arrays for each table
#     table1_headers = ["Sr. No.", "Software Component Name", "SWC Type", "Internal Behavior", "Runnable", "RTE Event", "Event Type"]
#     table2_headers = ["Sr. No.", "Port Name", "Port Type", "Interface Name", "Interface Type", "DataElement", "Application Data Type", "Description"]
#     table3_headers = ["Sr. No.", "Application Data Type", "Compu Method", "Compu Method Category", "Compu Scale", "Enum States", "Data Constraint", "Unit", "Mapped Implementation Data Type"]
#     table4_headers = ["Sr. No.", "Application Data Types", "Data Constraint", "Data Constraint Type", "Minimum Value", "Maximum Value"]
...
#         # Determine which table to search for based on table_value
#         if table_value == "table1":
#             headers = table1_headers
#         elif table_value == "table2":
#             headers = table2_headers
#         elif table_value == "table3":
#             headers = table3_headers
#         elif table_value == "table4":
#             headers = table4_headers
#         else:
#             print("Invalid table value provided.")
#             return None
        
#         # Find the position of the specified table
#         for row_num, row in current_sheet.iterrows():
#             if all(header in row.values for header in headers):
#                 # Found the header row
#                 print(f"Header row found at: {row_num}")
                
#                 # Specifically handle the "Application Data Type" case
#                 if header_phrase == "Application Data Type":
#                     # Force it to be the third column (index 2, which is 'C')
#                     e = "C8"
#                     print(f"Forced cell reference for 'Application Data Type': {e}")
#                     return e
                
#                 # For other headers, do the standard search
#                 try:
#                     header_index = headers.index(header_phrase)
#                     column_letter = chr(ord('A') + header_index)
#                     e = f"{column_letter}8"
#                     print(f"Cell 'e' found at: {e}")
#                     return e
#                 except ValueError:
#                     print(f"Header phrase '{header_phrase}' not found in headers.")
#                     return None
        
#         # If we've gone through the entire sheet without finding the headers
#         print("No matching headers found in this sheet.")

#     return None  # If no matching sheet or position is found
...
def CreatePorts():
    ...
    values_first_col, values_second_col = get_two_column_values(value_below_table2_header_2, 'Application Data Type')
    
    # Iterate through both columns simultaneously using zip
    for port_name, port_type in zip(values_first_col, values_second_col):
...
def createsharedinterfaces():
    SharedIFpkg = Utilities.GetElementByPath("/SharedElements/PortInterfaces")
