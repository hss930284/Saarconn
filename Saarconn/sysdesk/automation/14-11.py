    ...
    for row in range(start_row - 1, end_row):  # Subtract 1 from start_row because pandas is 0-indexed
        if row < current_sheet.shape[0]:
            ...
            if next_next_col_index < current_sheet.shape[1]:
                ...
                if not pd.isna(next_next_cell_value):  # Check if the cell is not empty
                    values_third_col.append(next_next_cell_value)
            else:
...
def createsharedinterfaces():
    ...
    # print(f"First value bcghbelow '{table2_headers[1]}' found at: {value_below_table2_header_4}")

    IF_name_col, IF_type_col, DE = get_three_column_values(value_below_table2_header_4, 'Compu Method Category')

    for IF_name, IF_type, DE in zip(IF_name_col, IF_type_col, DE):
        
        if IF_type == 'SenderReceiverInterface':
            SenderReceiverInterface(IF_name,DE)
...
def SenderReceiverInterface(currentIF_name, itsDE):
    ...
    # Utilities.SetDescription(currentSRIF, "TurnSwitchSensor signals")
