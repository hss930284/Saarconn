...
def createsharedinterfaces():
    ...
    # Iterate through the interface name, type, and DE columns
    for IF_name, IF_type, DE in zip(IF_name_col, IF_type_col, DE_col):
        
        if IF_type == 'SenderReceiverInterface':  # When a SenderReceiverInterface is found
            # If there's a previous interface collected, process it before starting a new one
            ...
            data_elements = [DE]  # Start collecting data elements for this interface
        
        elif IF_name == '' and current_IF_type == 'SenderReceiverInterface':  
            # When the IF_name is empty, it means we're still collecting DE for the current interface
            data_elements.append(DE)

        elif IF_type in ['NvDataInterface', 'ParameterInterface', 'ModeSwitchInterface', 'ClientServerInterface', 'TriggerInterface']:
    ...
    # After the loop ends, handle the last collected interface
    if current_IF_name and data_elements:
        SenderReceiverInterface(current_IF_name, data_elements)

# Function to handle other interfaces (extend this if needed)
def handle_other_interfaces(IF_name, IF_type, DE):
    if IF_type == 'NvDataInterface':
        NvDataInterface(IF_name, DE)
    elif IF_type == 'ParameterInterface':
        ParameterInterface(IF_name, DE)
...
def SenderReceiverInterface(currentIF_name, data_elements):
...
def NvDataInterface(currentIF_name, itsDE):
...
def ParameterInterface(currentIF_name, itsDE):
