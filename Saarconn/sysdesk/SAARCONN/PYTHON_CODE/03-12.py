...
def createcompumethod():
    ...
    for a, b, c, d,e in zip(CompuMethodName, CompuMethodCategory, CompuScaleOROffset, EnumStatesORLSB, Unit):
        currentcompumethod = CompuMethodsPackage.Elements.AddNewCompuMethod(a)
        CurrentUnit = commonPackage.Elements.Item(e)
        ...
        # Get the function from the switcher dictionary, defaulting to IDENTICAL
        func = switcher.get(b, handle_identical)  # Use b to get the function

        # Call the function with two arguments: c and d
        func(currentcompumethod, c, d, CurrentUnit)  # Pass the two arguments
...
def creatwDC():
    DataConstraintName,	DataConstraintType,	Min, Max = read_columns(ADT_Primitive, 'H', 'K')
