...
class FmiVersion(Enum):
    """
    This class defines the specified FMI version.
    """
    NONE = 0
    FMI2 = 1
    FMI3 = 2
...
def AddBusConnectorAndPorts(ecuInstance, commController, busConnectorName=None):
    ...
    elif commController.ElementType in ("ILinMaster", "ILinSlave"):
        busType = "Lin"
...
def AddModuleConfiguration(ecuConfiguration, bswModuleName, importToProject=False, moduleName=None, fmi=False):
    """
    ...
    if fmi and Options.FMUVersion == FmiVersion.FMI2:
        moduleDefinitionFile = SdApplication.EcuConfigurationManager.DefaultFmi2DefinitionFilename

        # Determine the package containing the BSW module definition.
        if bswModuleName in ("Rte", "MemMap"):
            definitionPackage = "/dSPACE"
        elif bswModuleName in ("Can", "FmiAd", "Os", "Platform", "Xcp"):
            definitionPackage = "/dSPACE_FMI2"
        else:
            definitionPackage = "/dSPACE_Sim"

    elif fmi and Options.FMUVersion == FmiVersion.FMI3:
        moduleDefinitionFile = SdApplication.EcuConfigurationManager.DefaultFmi3DefinitionFilename

        # Determine the package containing the BSW module definition.
        if bswModuleName in ("Rte", "MemMap"):
            definitionPackage = "/dSPACE"
        elif bswModuleName in ("Can", "FmiAd", "Os", "Platform", "Xcp"):
            definitionPackage = "/dSPACE_FMI3"
        else:
            definitionPackage = "/dSPACE_Sim"
        ...
        # Determine the package containing the BSW module definition.
        if bswModuleName in ("Rte", "MemMap", "Platform"):
            definitionPackage = "/dSPACE"
        else:
            definitionPackage = "/dSPACE_Sim"
...
def BuildForVeos(simSystem, vEcuName=None):
    ...
    if vEcuName != None:
        ...
        assert vEcu != None, f"V-ECU {vEcuName} does not exist"
    else:
        vEcu = None
...
def BuildModelContainerForVeos(vpProject, containerPath="", buildPath=None, xcpPort=None, debug=True, gcc=False):
    ...
    if not os.path.exists(containerPath):
        raise Exception(f"Model implementation container does not exist at path: {containerPath}")
    containerFileName = os.path.basename(containerPath)
    (vpuName, fileExt) = os.path.splitext(containerFileName)
    if fileExt.lower() not in (".vecu", ".sic", ".bsc", ".fmu"):
        raise Exception(f"Name of container file {containerFileName} does not end with valid extension (.vecu, .sic, .bsc, .fmu)")
    ...
    if buildStatus == VpEnums.BuildStatusEnum.Valid:
        print(f"Build of {vpuName} finished with status VALID")
    else:
        # Print build output and throw exception.
        if buildResult != None:
            print("Output of build: \n\n " + buildResult.BuildOutput)
        print("*** Build finished with status " + str(buildStatus))
        raise Exception(f"Build of {containerFileName} aborted with status {buildStatus}")
...
def BuildSicForVeos(vpProject, sicPath="Untitled.sic", buildPath=None, xcpPort=None):
    ...
    if not os.path.exists(sicPath):
        raise Exception(f"SIC does not exist at path: {sicPath}")
    sicFileName = os.path.basename(sicPath)
    (vpuName, sicExt) = os.path.splitext(sicFileName)
    if sicExt.lower() != ".sic":
        raise Exception(f"Name of SIC file {sicFileName} does not end with '.sic'")
    osaFile = vpProject.SystemFile
    if vpProject.Vpus.Item(vpuName):
        vpProject.RemoveVpu(vpuName)
        vpProject.Save(osaFile)
    ...
    if buildStatus == VpEnums.BuildStatusEnum.Valid:
        ...
        if buildResult != None:
            print("Output of build: \n\n " + buildResult.BuildOutput)
        print("*** Build finished with status " + str(buildStatus))
        raise Exception(f"Build of {sicFileName} aborted with status {buildStatus}")
...
def ConnectToSystemDesk():
    ...
    if SdApplication == None:
        ...
        SdApplication = win32com.client.Dispatch("SystemDesk.Application.2023-B")
        ...
        if bswAutomationDir not in sys.path:
        ...
        if scriptingDir not in sys.path:
            sys.path.insert(0, scriptingDir)
        # The enum definitions are needed later.
        import SystemDeskEnums # type: ignore # pylint: disable=import-outside-toplevel
...
def ConnectToVeosPlayer():
    ...
    if VpApplication is None:
        print("Opening COM connection to VEOS Player")
        try:
            VpApplication = win32com.client.Dispatch("VeosPlayer.Application.2024-A")
        except Exception:
            pass
        if not VpApplication:
            VpApplication = win32com.client.Dispatch("VeosPlayer.Application.2023-B")
        if not VpApplication:
            raise Exception("Could not dispatch VEOS Player 2024-A or 2023-B")
        ...
        if scriptingDir not in sys.path:
            sys.path.insert(0, scriptingDir)
        # The enum definitions are needed later.
        import VeosEnums # type: ignore # pylint: disable=import-outside-toplevel
...
def CreateProject(saveOldProject=False):
    ...
    print(f"Creating SystemDesk project {Options.ProjectName}")
    print(f"Using project file {projectFile}")
    # Create a new project.
    SdProject = SdApplication.CreateProject(projectFile)
    if SdProject == None:
        raise Exception(f"SystemDesk project '{projectFile}' could not be created.")
...
def CreateRequiredArtifact(implementation, dependencyName, \
    ...
    if "dsfxp" in dependencyName.lower():
        ...
        arEngineeringObject.RevisionLabels.Add("1.3.4")
        arEngineeringObject.Files.Add("<ProjectDir>\\_SharedFiles\\DsFxpLib+Vs1.3.4.pkc")
    else:
...
def CreateStandardMemorySection(implementation, sectionName):
    ...
    if swAddrMethodName.startswith("VAR"):
        description = f"Global or static variables (RAM{alignmentDesc})"
    elif swAddrMethodName.startswith("INTERNAL_VAR"):
        description = f"Global or static variables which are accessible by a calibration system (RAM{alignmentDesc})"
    elif swAddrMethodName.startswith("CONST"):
        description = f"Global or static constants (ROM{alignmentDesc})"
    elif swAddrMethodName.startswith("CALIB"):
        description = f"Global or static calibration parameters (Flash{alignmentDesc})"
    elif swAddrMethodName.startswith("CONFIG_DATA"):
        description = f"Global or static configuration data (Flash{alignmentDesc})"
    elif swAddrMethodName.startswith("CODE"):
        description = f"Global or static functions (ROM{alignmentDesc})"
    else:
...
def FindCommonPackage(arElement):
    ...
    while(True):
        if parent == None:
            ...
            if parent.ShortName in subPackageNames:
                parent = parent.Parent
            else:
                # Ok, package found.
                return parent
...
def GetNoUnit():
    ...
    """
    return GetElementByPath("/AUTOSAR/AUTOSAR_PhysicalUnits/Units/NoUnit") #changed by sushant
...
def ImportA2lFile(internalBehavior, a2lFilePath):
    ...
    try:
        ...
        SdApplication.BatchMode = oldBatchMode
    assert status, f"A2L import from file {a2lFilePath} failed. See Message Browser."
...
##    assert status, f"A2L export to file {a2lFilePath} failed. See Message Browser."
##
...
def ImportAutosarFiles(fileNames, importDiagrams=True, optionShowImportDialog=False):
    """
...
def ImportAutosarFilesAtProject(fileNames, importDiagrams=True, optionShowImportDialog=False):
    """
...
def OpenProject(saveOldProject=False):
    ...
    if not os.path.exists(projectFile):
        print(f"*** SystemDesk project {projectFile} does not exist.")
        return
    ...
    SdApplication.BatchMode = True
    try:
        print(f"\nOpening SystemDesk project {projectFile}")
...
def RunBswPlugin(bswModuleConfiguration, command, arg=None):
    ...
    try:
        ...
        raise Exception("Command '{command}' aborted with exception.") from e
...
def SaveProject(sdpFile=None):
    ...
    if sdpFile:
        print(f"Saving SystemDesk project as {sdpFile}")
        SdProject.SaveAs(sdpFile)
    else:
        print(f"Saving SystemDesk project {SdProject.Repository}")
        SdProject.Save()
...
def SetLinearCompuMethod(compuMethod, lsb=1.0, offset=0.0):
    ...
    if lsb >= 1.0:
        compuNumeratorCoeff0.MixedContent.AddStringContent(f"{offset}")
        compuNumeratorCoeff1.MixedContent.AddStringContent(f"{lsb}")
        compuDenominatorCoeff0.MixedContent.AddStringContent("1")
    else:
        compuNumeratorCoeff0.MixedContent.AddStringContent(f"{(offset/lsb)}")
        compuNumeratorCoeff1.MixedContent.AddStringContent("1")
        compuDenominatorCoeff0.MixedContent.AddStringContent(f"{(1.0/lsb)}")
...
def SetSwValueBlockSize(arElement, valBlkSize):
    ...
    """
    AssertIf((arElement.Category not in ("VAL_BLK", "ARRAY")), \
...
def SetFillColor(item, fillColor):
    ...
    if fillColor.lower() == 'orange':
        ...
        assert 0, f"Invalid color name '{fillColor}'"
...
def SetTextColor(item, textColor):
    ...
    if textColor.lower() == 'orange':
        ...
        assert 0, f"Invalid color name '{textColor}'"
...
def Isa(obj, typeName):
    ...
    """
    if "'" + typeName + "'" in str(obj.__class__):
        return True
    else:
        return False
...
def IsNumerical(obj):
    ...
    """
    if "'float'" in str(obj.__class__):
        return True
    elif "'int'" in str(obj.__class__):
        return True
    elif "'list'" in str(obj.__class__) and bool(obj):
...
def ThrowIfError(messages, command="Command"):
    ...
    """
    if not messages:
        msg = f"*** UNEXPECTED RESULT: Command '{command}' does not return a message object."
    ...
    if errorMessages.Count == 1:
        msg = f"{command} aborted with {firstError.MessageIdentifier}:\n{firstError.MessageText}\nSee Message Browser."
    else:
        msg = f"{command} aborted with errors. First message was {firstError.MessageIdentifier}:\n{firstError.MessageText}\nSee Message Browser."
    # Throw exception.
    raise Exception(msg)
...
def AssertEmpty(collection, collectionName):
    ...
    """
    if not collection:
        assert 0, f"The {collectionName} collection is not defined (NoneType)."
    if collection.Count == 0:
        assert 0, f"The {collectionName} collection at {collection.Parent.ShortName} must not be empty. The number of elements is {collection.Count}"
...
def AssertOneElement(collection, collectionName):
    ...
    if collection.Count != 1:
        assert 0, f"The {collectionName} collection at {collection.Parent.ShortName} must contain exactly one element. The number of elements is {collection.Count}"
...
def AssertZeroOrOneElement(collection, collectionName):
    ...
    if not collection:
        assert 0, f"The {collectionName} collection is not defined (NoneType)."
    if collection.Count > 1:
        assert 0, f"The {collectionName} collection at {collection.Parent.ShortName} must contain zero or one elements. The number of elements is {collection.Count}"
...
class _MiniProfiler():
    ...
    def DeltaTime(self, display=True):
        ...
        if display:
            PrintD(f"DeltaTime ={deltaTime:6.3f} sec")
        return deltaTime
    ...
    def TotalTime(self, display=True):
        ...
        if display:
            PrintD(f"TotalTime ={totalTime:6.3f} sec")
        return totalTime
