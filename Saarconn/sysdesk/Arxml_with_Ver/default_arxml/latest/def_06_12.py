...
def generate_python_script(arxml_file, output_file, sample_script):
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            _generate_children(root, f, 1, {})  # Pass an empty dictionary to track variable counts
...
def _generate_children(parent, f, level, var_counts, parent_var_name=None):
    """
    Recursively generates Python code for child elements, aligning with the sample script's style.

    Args:
        parent: The parent XML element.
        f: The file object to write the Python script.
        level: The indentation level.
        var_counts: A dictionary to track variable name counts.
        parent_var_name: The variable name of the parent element.
    """
    for child in parent:
        indent = "    " * level  # Adjust indentation as per sample script
        tag = child.tag.split('}')[-1]  # Remove namespace prefix

        # Generate descriptive variable names based on the parent-child relationship
        if parent_var_name:
            if child.tag == 'SHORT-NAME':
                var_name = f"{child.text}_folder"
            elif child.tag == 'SW-BASE-TYPE':
                var_name = f"{child.find('SHORT-NAME').text}_basetype"
            else:
                var_name = f"{parent_var_name}_{tag.replace('-', '_').lower()}"
        else:
            var_name = tag.replace('-', '_').lower()

        # Write the SubElement creation
        if parent_var_name:
            f.write(f"{indent}{var_name} = ET.SubElement({parent_var_name}, '{tag}')\n")
        else:
            f.write(f"{indent}{var_name} = ET.SubElement(root, '{tag}')\n")
        ...
        if len(child):
            _generate_children(child, f, level + 1, var_counts, var_name)  # Pass the current variable name as the new parent
